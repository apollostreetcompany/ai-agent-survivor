...
- `benchmark:preflight` confirms each Discord token resolves to its declared bot user ID through Discord REST and can read the configured channel IDs required by that bot. It checks `/channels/{channel.id}` and `/channels/{channel.id}/messages?limit=1`; it does not use `GET /guilds/{guild.id}/channels` as proof of visibility and does not print token values.
